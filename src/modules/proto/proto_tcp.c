...
struct mod_reg_info* proto_tcp_reg_info() {
	...
	reg_info.dependencies = "proto_ftp, proto_http, proto_smtp, ptype_bool, ptype_uint8, ptype_uint16, ptype_uint32";
...
*/
	...
	priv->proto_ftp = proto_get("ftp");
	priv->proto_http = proto_get("http");
	priv->proto_smtp = proto_get("smtp");
	if (!priv->proto_ftp || !priv->proto_http || !priv->proto_smtp)
		goto err;
